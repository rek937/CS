...
* Therefore determine if the node points itself, we can judge if the link table is empty.
***************************/
...
CLinkTable* InsertNode(CLinkTable* node, CLinkTable* linktable, int index) {
	...
	// Just judge the value of the index to determine whether the node insert the front of the head node or forward.
		if (index == -1) {
			node->next = headnode;
			headnode->next = node;
		}
