...
* just only judge if the next of haed node is NULL without cheching the prev of head node.
*****************************/
...
*****************************/
FLinkTable* InserrNode(FLinkTable* node, FLinkTable* linktable, int index) {
	FLinkTable* headnode = linktable;
	FLinkTable* currentnode = headnode->next;
	...
	// The link table is empty. There are 2 situations.
		...
		// The node intends to be stored in a position other than the next node of the head node, but it doesm't match the current state of the link table.
		else {
